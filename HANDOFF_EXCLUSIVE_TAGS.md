...
| 項目 | 状態 |
|---|---|
| ローカルDB `is_exclusive` | ✅ 再スクレイピング完了（55件中0件が専売 — DB内の既存記事には限定作品なし） |
| サーバーDB `is_exclusive` | ✅ 変更不要（全件0のまま） |
| WPタグ `DigiKet限定` | ✅ 0件で正しい |
| 判定ロジック | ✅ 修正済み（v13.7.2: 戻り値の受け漏れバグ修正。今後の新規取得分から正しく判定される） |
...
| `FANZA独占` | 12 | 12 | ✅ |
| `らぶカル独占` | 13 | 13 | ✅ |
| `DigiKet限定` | 0 | 0（確定） | ✅ 既存55件に限定作品なし。今後の新規取得分から正しく判定される |
| `FANZA専売`(誤名) | 0 | - | ✅ 除去済み |
