...
- **デプロイ対象**: `auto_post.py`, `novelove_core.py`, `novelove_fetcher.py`, `novelove_soul.py`, `novelove_ranking.py`, `novelove_writer.py`（nexus系は手動 `update_ui.py` を実行）
...
**.envに必須のキー（12個）:**
`GEMINI_API_KEY`, `WP_USER`, `WP_APP_PASSWORD`, `DMM_API_ID`, `DMM_AFFILIATE_API_ID`, `DMM_AFFILIATE_LINK_ID`, `DISCORD_WEBHOOK_URL`, `DLSITE_AFFILIATE_ID`, `DEEPSEEK_API_KEY`, `DIGIKET_AFFILIATE_ID`, `GSC_SERVICE_ACCOUNT_JSON`, `GSC_SITE_URL`
...
| ✅ 完了 | `auto_deploy.sh` のデプロイ対象に `novelove_ranking.py` / `novelove_writer.py` を追加（v15.0） |
