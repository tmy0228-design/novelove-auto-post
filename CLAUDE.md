...
- **統合DBファイル (新)**: `/home/kusanagi/scripts/novelove_unified.db`（gitignore対象）
- **バックアップ旧DB**: `/home/kusanagi/scripts/novelove.db` 等3つ（v18.0.0移行の安全装置として意図的に残置）
...
- `novelove.db` / `novelove_dlsite.db` / `novelove_digiket.db` (v18.0.0で役目を終えた旧DB。万が一の切り戻し用に意図的に残してあるため、**絶対に読み書きせず、また勝手に削除もしてはならない**)
...
| 🔜 次のタスク | （現在のタスクリストは空です。次の大型アップデート計画待ち） |
