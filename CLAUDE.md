...
6. **関数の戻り値や引数を変更した場合、全呼び出し元を確認する** — grepで全ファイル横断検索し、旧インターフェースの残骸を残さない
...
- `nexus_dashboard.py` / `nexus_rewrite.py` / `nexus_revive.py` / `nexus_healer.py` / `nexus_gsc.py`
...
**.envに必須のキー（11個）:**
`GEMINI_API_KEY`, `WP_USER`, `WP_APP_PASSWORD`, `DMM_API_ID`, `DMM_AFFILIATE_API_ID`, `DMM_AFFILIATE_LINK_ID`, `DISCORD_WEBHOOK_URL`, `DLSITE_AFFILIATE_ID`, `DEEPSEEK_API_KEY`, `DIGIKET_AFFILIATE_ID`, `GSC_SERVICE_ACCOUNT_JSON`
...
| ✅ 完了 | 5分タイムアウト時の過剰な緊急停止をスキップ＆リトライに変更（v14.5.0） |
| ✅ 完了 | Discord通知のFANZA/DMM/らぶカル在庫件数が全件合算されていたSQL修正（v14.5.0） |
| ✅ 完了 | サーバーから消えた `gsc-service-account.json` / `auto_deploy.sh` / `sync_db_wp.py` / `update_affiliate_text.py` を復元（v14.5.0） |
