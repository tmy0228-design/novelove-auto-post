...
- `novelove_bluesky.py`
- `nexus_dashboard.py` / `nexus_rewrite.py` / `nexus_revive.py` / `nexus_healer.py` / `nexus_gsc.py` / `nexus_purge.py` / `nexus_purge_dead.py`
- `novelove_ranking.py`
...
**.envに必須のキー（15個）:**
`GEMINI_API_KEY`, `WP_USER`, `WP_APP_PASSWORD`, `DMM_API_ID`, `DMM_AFFILIATE_API_ID`, `DMM_AFFILIATE_LINK_ID`, `DISCORD_WEBHOOK_URL`, `DLSITE_AFFILIATE_ID`, `DEEPSEEK_API_KEY`, `GSC_SERVICE_ACCOUNT_JSON`, `GSC_SITE_URL`, `OPENROUTER_API_KEY`, `BLUESKY_HANDLE`, `BLUESKY_APP_PASSWORD`
