...
| `nexus_purge_dead.py` | 死に記事自動パージ（GSCデータベースの未インデックス/低トラフィック検知・殿堂入り保護） |
...
novelove_core.py  ←  魂(soul)とインフラ(core)。全モジュールの基盤
    ...
    ├── novelove_bluesky.py  (Bluesky自動投稿)
...
- **統合DBファイル**: `/home/kusanagi/scripts/novelove_unified.db`（gitignore対象）
- **認証ファイル**: `/home/kusanagi/scripts/gsc-service-account.json`（gitignore対象）

### cron設定（変更前に必ず確認）

```
*/15 * * * *  auto_post.py                          → 自動投稿（15分ごと。実効間隔はDB在庫に応じた動的cooldown。DeepSeekピーク帯JST 10–13/15–19は最低60分）
0 9 * * *     auto_post.py --ranking                → ランキング記事1回目（BL）9時
30 9 * * *    auto_post.py --ranking                → ランキング記事2回目（TL）9:30、BL投稿済みを検知しTLを投稿
10 9 * * 1    novelove_curator.py --force --genre=BL → BLまとめ（月曜9:10）
10 9 * * 3    novelove_curator.py --force --genre=BL → BLまとめ（水曜9:10）
10 9 * * 5    novelove_curator.py --force --genre=TL → TLまとめ（金曜9:10）
10 9 * * 0    novelove_curator.py --force --genre=TL → TLまとめ（日曜9:10）
...
5. **Windows環境由来の「文字化け（Mojibake）」でパニックにならないこと**
   ...
   - （※外部サイト由来の文字コードは `novelove_fetcher.py` の `_response_text()` で Content-Type → UTF-8 を優先し、文字化けと判定した本文は採用しない。DMM商業のみ本文HTMLが無い場合に JSON-LD を使う。詳細は `SPECIFICATIONS.md` 第2章）
   - 文字化けが気になって作業に支障が出る場合は、PowerShell内で一時的に `chcp 65001` （UTF-8モード）を実行してから各種コマンドを実行すること。

6. **同一作品の重複掃除・サイト優先は DLsite びいき**
...
7. **WordPress記事の削除はゴミ箱のみ（force禁止）**
   ...
   - 正規手順はゴミ箱送り（`wp post delete ID`、ダッシュボード削除、purge も同様）。誤爆時に戻せるようにする。
...
9. **WP REST の `status` を配列で渡して既存投稿を探すな** — 💀 **v21.5.1バグ**: ランキング固定スラグの上書きで `status=['publish','draft',...]` を渡した結果、WPが空配列を返し毎回新規作成 → `*-ranking-2` が量産された。既存スラグ検索は **`status=any`（認証必須）→ だめなら status 省略**。詳細は `SPECIFICATIONS.md` の固定スラグ節と `CHANGELOG.md` v21.5.1。

### 🏷️ タグ分類モデル（v21.7.0・重要）

全WPタグに種別印 `nv_tag_type`（term meta）が付く：`ai / cast / circle / author / site / reviewer / system / exclusive`。
- **index閾値**: 声優/サークル/作者(cast/circle/author)は**10件**、他は**5件**（`functions.php` の `nv_tag_index_threshold()`。noindex・sitemap・JSON-LDの単一ソース）。
- **タグ順（投稿もリライトも共通）**: サイト → 同人/商業 → AI(+専売) → 声優 → サークル → 作者 → 担当者。`auto_post.py` と `nexus_rewrite.py` は**必ず同順**に保つこと。
- **出処タグ `同人`/`商業`**: `nv_tag_type=system`。気分プール除外。タグページ説明は自動テンプレではなく手動執筆を推奨。
- **年齢【凍結】**: WPに `R-18`/`全年齢` タグは付けない。Blueskyの `porn` ラベルのみ `classify_is_r18` で粗分け（迷い時は付ける）。
- ナビ「同人／商業で探す」は配信サイトの直後（PC/スマホ）。「クリエイターで探す」は index対象(10件+)のみ。「今の気分は？」は ai タグのみ（人名/サークルは除外）。
- **出版社/レーベルはタグ化しない**（`サークル:` `著者:` のみ）。表記ゆれは `normalize_entity_key` で名寄せ、表示名は元表記維持。`/`・`・`・`&` は分割しない。
- ナビ「クリエイターで探す」は index対象(10件+)のみ表示。「今の気分は？」は ai タグのみ（人名/サークルは除外）。
...
- `novelove_bluesky.py`
- `nexus_dashboard.py` / `nexus_rewrite.py` / `nexus_revive.py` / `nexus_healer.py` / `nexus_gsc.py` / `nexus_purge_dead.py`
- `novelove_ranking.py` / `novelove_curator.py`
...
**データベース:**
- `novelove_unified.db` (現在稼働している唯一の統合正本DB)

**.envに必須のキー（15個）:**
`GEMINI_API_KEY`, `WP_USER`, `WP_APP_PASSWORD`, `DMM_API_ID`, `DMM_AFFILIATE_API_ID`, `DMM_AFFILIATE_LINK_ID`, `DISCORD_WEBHOOK_URL`, `DLSITE_AFFILIATE_ID`, `DEEPSEEK_API_KEY`, `GSC_SERVICE_ACCOUNT_JSON`, `GSC_SITE_URL`, `OPENROUTER_API_KEY`, `BLUESKY_HANDLE`, `BLUESKY_APP_PASSWORD`, `SSH_PASS`
