...
| `nexus_purge.py` | 低品質記事の自動削除（スコア再審査ベース） |
...
**.envに必須のキー（15個）:**
`GEMINI_API_KEY`, `WP_USER`, `WP_APP_PASSWORD`, `DMM_API_ID`, `DMM_AFFILIATE_API_ID`, `DMM_AFFILIATE_LINK_ID`, `DISCORD_WEBHOOK_URL`, `DLSITE_AFFILIATE_ID`, `DEEPSEEK_API_KEY`, `DIGIKET_AFFILIATE_ID`, `GSC_SERVICE_ACCOUNT_JSON`, `GSC_SITE_URL`, `OPENROUTER_API_KEY`, `BLUESKY_HANDLE`, `BLUESKY_APP_PASSWORD`
...
| ✅ 完了 | API基盤刷新v17.0.0〜v17.1.0: DeepSeek→OpenRouter/Grokハイブリッド→Grok-4.1-Fast全記事統一 |
| ✅ 完了 | ペルソナ駆動型キャラv17.3.0〜v17.4.0: スラング直接指定廃止・公式サイト準拠のペルソナ全面刷新・口癖ランダム挿入実装 |
| ✅ 完了 | DeepSeek-V4-Flash全面移行v17.7.0 (2026-04-24): Grok→deepseek-v4-flash完全移行・max\_tokens 2000→3000・途切れ検知追加 |
| ✅ 完了 | フォールバックv4-flash統一・max\_tokens引上げv17.7.1 (2026-04-25): OpenRouterフォールバックもdeepseek-v4-flashに統一・max\_tokens 3000→8000 |
| ✅ 完了 | プロンプト圧縮・WP slug検索修正・DB大文字小文字不一致修正v17.8.0〜v17.8.7 (2026-04-25): 記事品質向上・slug誤爆防止・DB検索バグ根治 |
| ✅ 完了 | DB統合アーキテクチャ刷新v18.0.0 (2026-04-26): 旧3DB構成を `novelove_unified.db` の1本に完全統合。クロスDB重複排除とダッシュボード高速化を実現。 |
| ✅ 完了 | 自動デプロイ刷新 (2026-04-26): `auto_deploy.sh` を個別ダウンロードから `git reset --hard` & `git pull` の一括同期方式に刷新。全ファイルが自動デプロイ対象に。 |
| ✅ 完了 | プロンプト品質向上・投稿ペース最適化v18.2.0 (2026-05-08): スコア5上限900字・スコア4素材量連動200〜600字・おすすめ汎用化禁止・SEOタイトルTAGS参照・cooldown 45分・cron 25分 |
| ✅ 完了 | Bluesky自動投稿連携v18.4.0 (2026-05-10): novelove_bluesky.py新設・WP投稿成功後の自動連携・画像/R18ラベル/Threadgate/タグフィルタ/セッションキャッシュ対応 |
| ✅ 完了 | R-18判定抜本改修v18.4.0 (2026-05-10): 旧エロワードキーワード検索を全面廃止。DMM=全年齢/FANZA=R18/DigiKet=R18/DLsite=icon\_ADLバッジ判定。novelove\_writer.pyの投稿時判定もブランドベースに統一。既存DB在庫29件修正済み |
| ✅ 完了 | 死に記事自動パージ＆殿堂入り保護v18.6.0 (2026-05-19): nexus_purge_dead.py新設。未インデックス45日超/低トラフィック60日超の死に記事を自動削除。クリック実績1回以上の記事にis_protected永久保護フラグを自動付与しパージ対象から完全除外 |
| ✅ 完了 | Bluesky茉莉花投稿文・あらすじ引用の自然化v18.5.1 (2026-05-19): 茉莉花の口癖強要プロンプトの排除とあらすじ優先の動的文字数調整、Bluesky引用でのライター名混入を防ぐ商品説明フォールバック実装 |
| ✅ 完了 | Bluesky茉莉花SNS担当化・投稿頻度最適化v18.5.0 (2026-05-19): DeepSeekによる茉莉花キャラコメント生成・facet正式タグ実装・ハイブリッドフィルタ（DLsite/FANZA/らぶカル=専売+スコア5のみ/DigiKet/DMM=スコア5のみ）で1日約13件に最適化 |
| 🔜 次のタスク | Blueskyカスタムフィード登録（投稿実績が溜まった段階で手動対応） |
