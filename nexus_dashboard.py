    ...
    if detail_pid:
        ...
        if not match.empty:
            ...
            if row.get("status") == "published":
                ...
                MOOD_OPTIONS = {
                    "ランダム": None,
                    "熱量高め（あらすじでテンション）":   "今回はあらすじを見ただけで熱量が上がっている。いつもよりテンション高めに語る。",
                    "冷静分析（本音が漏れる）":           "今回は冷静に魅力を整理しつつも、総評で本音が漏れてしまう。",
                    "布教欲（名作を発掘した興奮）":       "今回は「まだ知られていない名作」を発掘した興奮がある。誰かに伝えたくてたまらない熱量を持つ。",
                    "じわじわ期待（噛みしめる）":         "今回はじわじわと期待が膨らむタイプ。噛みしめるように語る。",
                    "衝撃（設定だけで勢い）":             "今回は冒頭の設定だけで心を持っていかれた衝撃がある。勢いのまま語る。",
