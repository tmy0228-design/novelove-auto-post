...
- **サーキットブレーカー**: 連続エラー3回 or 10分超過で自動停止 → Discord緊急通知（`emergency_stop.lock`生成）
- **関所（冷却機能）**: サーバー高負荷やSQLiteのロックを避けるため、起動後最低1秒以上のインターバルを強制する冷却ロジックを実装。
