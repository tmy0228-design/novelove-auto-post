...
> ※ DigiKetはv20.0.0で巡回・通知を完全削除。既存記事は残存。

Novelove（ノベラブ）のコンテンツ生成および自動投稿を行うエンジン一式です。
BL・TL の女性向け作品（漫画・小説・同人）を全自動で紹介記事化して WordPress に投稿します。

## 🚀 システム概要
- **目的**: DLsite, DMM.com(一般), らぶカル(Lovecal) の新着・ランキングを取得し、**DeepSeek V4-Flash**（直接API / OpenRouterフォールバック）で紹介記事を生成して WordPress に自動投稿する。※DigiKetはv20.0.0で巡回・通知を完全削除。既存記事は残存。
- **主要言語**: Python 3.x
...
### 8. らぶカル source_db 修正とリファクタリング (v20.0.2)
...
- **DigiKet保留在庫の除外**: 巡回停止に伴い、DB上の古い DigiKet 保留在庫 6 件を一括で除外ステータスへ移行。
