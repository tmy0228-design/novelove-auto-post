# Novelove 自動投稿システム 技術詳細仕様書 (v8.9.2)
...
```mermaid
graph TD
    A[システム起動/Fetch完了] --> B{全ジャンル巡回開始}
    B --> C{対象ジャンルに pending があるか?}
    C -- Yes (最優先) --> D[最新の pending 記事を投稿]
    C -- No --> E{watching 枠があるか?}
    E -- Yes (保険枠) --> F[最古作品を抽出/再審査]
    F -- 合格 --> G[投稿成功]
    F -- 不合格 --> H[excluded 移行 ＆ 次のジャンルへ]
    E -- No --> I[次のジャンルへ]
    D --> J[成功終了 ＆ 次回開始位置保存]
    G --> J
    I --> K{全ジャンル確認済み?}
...
### 1-2. ジャンル巡回 (Polling/Rotation)
- **挙動**: 1回の実行で投稿が成功するまで（または全 10 ジャンルをスキャンの終えるまで）、`pending` および `watching` を順番に確認します。
- **目的**: 投稿ネタがある限り、確実に 1 時間に 1 件の投稿が行われる「空振り防止」のための仕組みです。

### 1-3. 保険枠 (Insurance/Queue Processing)
- **挙動**: 該当ジャンルに `pending`（新着合格）がない場合、`status='watching'` かつ `release_date`（発売日）が今日以前の作品を 1 件選び、再審査後に投稿します。一度再審査して不合格になったものは、即座に `excluded` （除外）となる「一発勝負」方式です。
...
- **DigiKet**: XML API (RSS) はあらすじが短いため、投稿・審査前に必ず商品詳細ページをスクレイピングし、作品内容を全文取得・補完します。
...
1. **新着取得数**: FANZA/DMM API の `hits` を `50` に設定（DLsite は新着ページ全件）。
2. **先行画像チェック**: `_check_image_ok`。placeholder (Now Printing) の場合は AI を呼び出しません。
3. **先行発売日チェック**: 発売まで 8日以上 ある場合は AI を呼び出しません。
4. **あらすじ文字数**: 50文字以下は審査前に保留。
...
## 🔔 10. Discord通知仕様 (Discord Notifications)

### 10-1. 投稿成功通知
投稿が完了するたびに、以下のフォーマットで通知を送信します。
- **形式**: `✅ [サイト名] [ジャンル名] {post_label}成功！`
- **内容**: タイトル、AIスコア、統計（本日の件数、文字数）、公開URL。

### 10-2. 異常検知アラート（緊急）
サイト運営に支障をきたす可能性のある異常を検知した場合、即座に警告を送信します。
- **トリガー**: 作品詳細ページからあらすじが取得できなかった場合（かつ除外対象でない場合）。
- **目的**: サイトの構造変更やスクレイピング防止策の発動をいち早く察知し、エンジニアが対応できるようにするため。
- **警告内容**: ⚠️ **あらすじ取得失敗（サイト構造変化の可能性あり）**、対象作品名、URL。

---
**最終更新日**: 2026-03-15 (v8.9.2)
