...
def build_prompt(target, reviewer, mask_level=0, is_novel=False, is_guest=False, mood="", ai_score=4, original_tags="", is_exclusive=False, pattern="A"):
    ...
    if random.random() < 0.1:
        intro_rule = f"冒頭の挨拶1〜2文だけで、あなたのキャラクター設定にある身の上話（例: {reviewer.get('greeting', '')}）を自然に絡めること。ただし身の上話は短く留め、すぐに作品の紹介・リアクションに移ること。"
    else:
...
5. {voice_hint}
6. 「〜文字で書きます」「布教します」「タスク完了」のようなAIのメタ発言や指示の自己申告は絶対に禁止。最初から最後まで完全にキャラクターとして振る舞い、AIであることを悟らせないこと。
7. 記事本文（<p>タグ）では、あらすじ情報から「存在しない設定やキャラクター」を創作（ハルシネーション）して文字を水増しすることは絶対禁止。
8. h2見出しは毎回異なる切り口で書くこと。「○○に迫る」「○○が紡ぐ」のようなテンプレ表現は避けること。
9. 記事本文は、スマホでの読みやすさを重視し、適宜複数の <p> タグに分割するか、<br> タグを用いて改行してください。1つの <p> タグに長文を詰め込みすぎないこと。
{pattern_rules}
【対象作品情報】
タイトル: {safe_title}
