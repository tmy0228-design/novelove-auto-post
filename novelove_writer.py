...
def _evaluate_article_potential(title, description, original_tags=""):
    ...
    運用上: 3以下は不採用、4=標準記事、5=深掘り記事。
    """
    if not description or len(description.strip()) < 50:
        return 2  # 情報が少なすぎる場合はAPIを叩かずに即終了

    prompt = f"""
以下のタイトルとあらすじ（と属性があれば属性）を読み、「嘘や補完なしで、充実した紹介記事が書ける情報量があるか」を1〜5点で評価してください。
迷ったら無難に4へ寄せず、下のチェックリストで判定してください。

【有用な情報（加点）】
・世界観・舞台・シチュエーションの説明
・人物の立場・性格・関係性（誰が誰に何をするか）
・フェティッシュな属性・性癖、プレイや展開の具体
・見どころになる対立・制約・感情の動き

【ノイズ（無視）】
・ファイル形式・収録分数・価格・特典・同梱・URL
・サークル／スタッフ名の羅列、他作品の宣伝

【スコア5に必要な情報（深掘り記事〜約2000字向け）】
次のうち、本文ネタとして使える具体がおおよそ3つ以上そろっていること。
1. 舞台／シチュが説明できる
2. 人物とその関係・立場が分かる
3. プレイ・属性・展開の見どころが複数ある（箇条書きでも可）
4. 上記がメタ情報ではなく物語・体験の中身として書かれている
→ そろっているなら5。短文の煽りやタイトル言い換えだけでは5にしない。

【各点の目安】
5: 上の5条件を満たし、補完なしで深い紹介が書ける
4: 手堅い紹介（約1000字）には足りるが、深掘り材料は足りない／標準的
3: 魅力はあるが情報不足。短い紹介なら書ける程度
2: 薄い、またはノイズが大半
1: 判定不能・外国語

出力形式: 1〜5の数字1文字のみ（説明文禁止）
...
"""
    messages = [
        {"role": "system", "content": "あなたはプロの編集者です。情報量だけで基準どおりに採点し、曖昧なときはチェックリストを優先してください。"},
    ...
    # 説明付き出力のとき最初の数字を誤爆しないよう、最後の1-5を採用
...
def build_prompt(target, reviewer, mask_level=0, is_novel=False, is_guest=False, mood="", ai_score=4, original_tags="", is_exclusive=False, pattern="A"):
    ...
    v21.7.29: 本文＝事実／吹き出し＝予想可／タグ枠埋め禁止を明示（HTML骨格は不変）。
    """
    ...
    if is_voice:
        voice_rules = (
            ...
            "「コマ」「見開き」「絵」「描画」「読む」「ページ」等の漫画・小説表現は使わないこと。"
    ...
    tag_line = (
        f"\nテーマ傾向（※参考のみ。本文・見どころ・おすすめへの転記・列挙・枠埋めは禁止）: {original_tags}"
    ...
    # HTML骨格（見出し名・吹き出し個数・リスト個数）は固定。指示文だけ事実密度に寄せる。
    _intro5 = "あらすじ・世界観・テーマ要素を、あらすじの情報量に合わせて解説（目安400〜900字。無い要素は補完しない。同じ事実の言い換え連発禁止）。"
    _char5 = "あらすじにある人物の性格・立場・関係の事実の範囲で分析（目安400〜900字。無い設定は創作しない）。"
    _midokoro_li = "あらすじの事実に基づき具体的に。属性タグ名の羅列やタグだけで埋めるのは禁止。"
    _osusume_li = "あらすじの具体要素から導出した層。「感動的なストーリーが好きな方」等の汎用表現禁止。属性タグ名だけで埋めない。"

    if ai_score >= 5:
        if pattern == "B":
            pattern_rules = "6. h3見出しはあらすじの事実に基づくこと。存在しない設定の見出しは禁止。各セクションは対応する事実のみで書く。"
            html_structure = f"""
{chat_open}（60〜110字。作品への自然なリアクションで書き出す。予想・期待OK）{chat_close}
<h2>（作品の魅力を引き出す見出し）</h2>
<p>{_intro5}</p>
{chat_open}（50〜90字。リアクション。予想・期待OK）{chat_close}
<h2>キャラクターの魅力と関係性</h2>
<p>{_char5}</p>
{chat_open}（50〜90字。リアクション。予想・期待OK）{chat_close}
<h3>（魅力ポイント1。事実に基づく見出し）</h3>
<p>あらすじの情報に基づき200〜350字で深掘り。無い要素は補完しない。</p>
<h3>（魅力ポイント2。事実に基づく見出し）</h3>
<p>あらすじの情報に基づき200〜350字で深掘り。無い要素は補完しない。</p>
{chat_open}（120〜200字。熱い総評。予想・期待OK）{chat_close}
"""
        elif pattern == "C":
            pattern_rules = "6. Q&Aの質問はあらすじから答えられる自然な疑問のみ。回答もあらすじの事実のみで書く。タグで埋めない。"
            html_structure = f"""
{chat_open}（60〜110字。作品への自然なリアクションで書き出す。予想・期待OK）{chat_close}
<h2>（作品の魅力を引き出す見出し）</h2>
<p>{_intro5}</p>
...
<p>A. あらすじの事実のみで回答。200〜300字。創作禁止。</p>
...
"""
            ...
            pattern_rules = "6. 引用はあらすじ原文から一言一句変えずにコピー。改変・創作した引用は禁止。"
            html_structure = f"""
{chat_open}（60〜110字。作品への自然なリアクションで書き出す。予想・期待OK）{chat_close}
<h2>（作品の魅力を引き出す見出し）</h2>
<p>{_intro5}</p>
...
<p>この引用がなぜ読者の心を捉えるのか、あらすじの事実の範囲で300〜500字で語る。無い展開は創作しない。</p>
{chat_open}（120〜200字。熱い総評。予想・期待OK）{chat_close}
"""
        else:  # pattern == "A"
            pattern_rules = "6. 見どころ・おすすめはあらすじの事実から。テーマ傾向やTAGS名だけで枠を埋めない。汎用「感動的なストーリーが好きな方」は禁止。"
            html_structure = f"""
{chat_open}（60〜110字。作品への自然なリアクションで書き出す。予想・期待OK）{chat_close}
<h2>（作品の魅力を引き出す見出し）</h2>
<p>{_intro5}</p>
...
<h2>見どころ</h2>
<ul>
  <li><strong>（魅力1）</strong>：{_midokoro_li}</li>
  <li><strong>（魅力2）</strong>：{_midokoro_li}</li>
  <li><strong>（魅力3）</strong>：{_midokoro_li}</li>
</ul>
<h2>こんな人におすすめ</h2>
...
{chat_open}（120〜200字。熱い総評・布教。予想・期待OK）{chat_close}
"""
    elif ai_score == 4:
        if pattern == "B":
            pattern_rules = "6. h3見出しはあらすじの事実に基づくこと。存在しない設定の見出しは禁止。各セクションは対応する事実のみで書く。薄い箇所は短く止める。"
            html_structure = f"""
{chat_open}（60〜110字。作品への自然なリアクションで書き出す。予想・期待OK）{chat_close}
<h2>（作品の魅力を引き出す見出し）</h2>
<p>あらすじ・世界観・テーマ要素をあらすじの情報量に合わせて200〜600字で解説。情報がない要素は補完しない。同じ事実の言い換え連発禁止。</p>
{chat_open}（50〜90字。リアクション。予想・期待OK）{chat_close}
<h3>（魅力ポイント1。事実に基づく見出し）</h3>
<p>あらすじの情報に基づき150〜250字で深掘り。無い要素は補完しない。</p>
<h3>（魅力ポイント2。事実に基づく見出し）</h3>
<p>あらすじの情報に基づき150〜250字で深掘り。無い要素は補完しない。</p>
{chat_open}（120〜200字。熱い総評。予想・期待OK）{chat_close}
"""
        elif pattern == "C":
            pattern_rules = "6. Q&Aの質問はあらすじから答えられる自然な疑問のみ。回答もあらすじの事実のみ。タグで埋めない。薄い箇所は短く止める。"
            html_structure = f"""
{chat_open}（60〜110字。作品への自然なリアクションで書き出す。予想・期待OK）{chat_close}
<h2>（作品の魅力を引き出す見出し）</h2>
<p>あらすじ・世界観・テーマ要素をあらすじの情報量に合わせて200〜600字で解説。情報がない要素は補完しない。同じ事実の言い換え連発禁止。</p>
{chat_open}（50〜90字。リアクション。予想・期待OK）{chat_close}
<h3>Q. （あらすじから答えられる疑問1）</h3>
<p>A. あらすじの事実のみで回答。150〜250字。創作禁止。</p>
<h3>Q. （あらすじから答えられる疑問2）</h3>
<p>A. あらすじの事実のみで回答。150〜250字。創作禁止。</p>
<h3>Q. （あらすじから答えられる疑問3）</h3>
<p>A. あらすじの事実のみで回答。150〜250字。創作禁止。</p>
...
<p>この引用がなぜ読者の心を捉えるのか、あらすじの事実の範囲で200〜350字で語る。無い展開は創作しない。</p>
{chat_open}（120〜200字。熱い総評。予想・期待OK）{chat_close}
"""
        else:  # pattern == "A"
            pattern_rules = "6. 見どころ・おすすめはあらすじの事実から。テーマ傾向やTAGS名だけで枠を埋めない。汎用「感動的なストーリーが好きな方」は禁止。薄い箇所は短く止める。"
            html_structure = f"""
{chat_open}（60〜110字。作品への自然なリアクションで書き出す。予想・期待OK）{chat_close}
<h2>（作品の魅力を引き出す見出し）</h2>
<p>あらすじ・世界観・テーマ要素をあらすじの情報量に合わせて200〜600字で解説。情報がない要素は補完しない。同じ事実の言い換え連発禁止。</p>
...
"""
        ...
        # スコア3以下：コンパクト記事（現状ゲートでは不採用だがセーフティネット）
        pattern_rules = "6. 見どころはあらすじの事実のみ。最大2点。書ける事実が1点なら1点で完結。タグで埋めない。"
        html_structure = f"""
{chat_open}（40〜60字。作品への自然なリアクション。予想・期待OK）{chat_close}
<h2>（作品の魅力を一言で表す見出し）</h2>
<p>あらすじ・世界観を200〜400字で簡潔に解説。事実のみ。補完しない。</p>
<h2>見どころ</h2>
<ul>
  <li><strong>（魅力1）</strong>：あらすじの事実のみ。簡潔に。</li>
</ul>
{chat_open}（60〜80字の総評。予想・期待OK）{chat_close}
"""
    # キャッチフレーズ指示（20%確率）
...
【ルール】
1. 吹き出しコメント＝あなたの口調全開。予想・期待・妄想・推し感情は吹き出しでOK（「〜しそう」等）。本文（h2/p/ul/li）＝標準語・ですます調の客観パート。本文と吹き出しは混ぜない。一人称はキャラ設定どおりに固定（作中セリフの「」引用のみ例外）。
2. あなたはあらすじや公開情報だけを頼りに紹介している立場。本文では事実のみ断定。実体験を装う表現、あらすじに無い設定・キャラ名・展開の創作は禁止。同じ事実の言い換え連発で文字数を稼がない。情報がない要素は補完しない。
3. テーマ傾向は参考のみ。転記・属性名の列挙・「○○属性が好きな方」だけで枠を埋める行為は禁止。自然な文脈で語る場合のみ可。直接的な性的単語は官能的な比喩に。
4. 本文の各セクション（h2直下のpタグ）は1段落あたり120〜180字を目安に、内容の区切りで複数の<p>タグに分けること。400字以上を1つの<p>に詰め込まない。
5. 独自の略語・当て字などの造語は禁止。一般に定着した表現のみ使用すること。
{pattern_rules}
...
OPENROUTER_FALLBACK_MODEL = "deepseek/deepseek-v4-flash"  # v17.7.1: deepseek-chatからv4-flashへ統一（品質維持）
...
def call_deepseek(prompt, model_id=None):
    ...
    for attempt in range(3):
        ...
        t_start = time.time()
        # v17.8.11: max_tokens=16000（DeepSeek V4 Flash 上限に余裕）
        # v21.7.20: 執筆も thinking OFF（推論トークン課金抑制。ペルソナは soul の声DNAで担保）
        text, error_type = _call_deepseek_raw(
            messages, max_tokens=16000, temperature=0.8, model_id=_model, thinking_disabled=True
        )
