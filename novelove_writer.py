...
"""
    ...
    # v19.0.0: TAGSブロックをボイス/非ボイスで切り替え（f-string外で組み立て、バックスラッシュ制約を回避）
    if is_voice:
        tags_block = (
            "TAGS: （作品に合うものを最大3つ、カンマ区切り。該当なしは「なし」）\n"
            "音声特有: ASMR/バイノーラル/ダミーヘッドマイク/ボイスドラマ/シチュエーションボイス/ささやき/添い寝/耳かき/耳舐め/吐息/囁き声\n"
            "共通テーマ: 執着攻め/ヤンデレ/年の差/一途/運命/溺愛/独占欲/年下攻め/幼なじみ/主従/身分差/ハッピーエンド"
        )
    else:
        tags_block = (
            "TAGS: （作品に合うものを最大3つ、カンマ区切り。該当なしは「なし」）\n"
